...
def collect_metrics(models: Iterable[str], subject: Optional[str]) -> dict:
    ...
    for model_name, items in buckets.items():
        ...
        runtimes = [
            item.runtime_seconds or 0.0 for item in items if item.runtime_seconds
        ]
        ...
        metrics[model_name] = {
            ...
            "throughput": (
                (mean(durations) / mean(runtimes)) if durations and runtimes else 0.0
            ),
            "rtf": (
                (mean(runtimes) / mean(durations)) if durations and runtimes else 0.0
            ),
        }
    return metrics
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Resume métricas de modelos Whisper basadas en la base de datos."
    )
    parser.add_argument(
        "--models",
        nargs="*",
        help="Filtra por modelos concretos (ej. large-v2 large-v3).",
    )
    parser.add_argument(
        "--subject", help="Limita el análisis a una asignatura concreta."
    )
    parser.add_argument(
        "--export",
        type=Path,
        help="Ruta opcional para exportar las métricas en formato JSON.",
    )
